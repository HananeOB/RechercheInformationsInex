...
for q in query_list :
    ...
    ##print(num_m[-1], doc_m[-1], sep=" : ")

    # Creates vectorial matrix with corresponding weights :
    vectorizer = tf_idf(stop_words='english')
    ...
    query_output = cosine_similarity(matrix[-1], matrix[0:-1]) # query_output is a list inside a list (je ne sais pas pq)
    print(query_output)
    # Sorts queries :
    matrix_resultats = [(index, content) for index, content in enumerate(query_output[0]) ]
    print(matrix_resultats)
    print(len(matrix_resultats))
    # C = sorted(matrix_resultats, key=lambda tup: tup[1], reverse= True)[0:1500]        # Maximum results = 1500
    # rang = 1
    # for e in C :
    # print(len(matrix_re))    
